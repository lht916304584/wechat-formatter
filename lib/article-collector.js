...
async function collectArticle({ url, apiKey, baseUrl, fetchImpl = fetch }) {
  ...
  const endpoints = collectTikHubBases(baseUrl).flatMap((base, index) => [
    { url: `${base}/api/v1/wechat_mp/web/fetch_mp_article_detail_html?url=${encoded}`, mode: 'html', via: index === 0 ? 'tikhub-html' : 'tikhub-html-alt', attempts: 1 },
    { url: `${base}/api/v1/wechat_mp/web/fetch_mp_article_detail_json?url=${encoded}`, mode: 'json', via: index === 0 ? 'tikhub-json' : 'tikhub-json-alt', attempts: 1 },
  ]);
