...
async function requestTikHub(endpoint, apiKey, fetchImpl, mode = 'html') {
  ...
  if (!res.ok) {
    ...
    error.retryable = res.status === 400 && /request failed|please retry|请求失败|重试/i.test(`${apiMessage} ${details}`);
    throw error;
  }
  if (payload && typeof payload === 'object' && payload.code && payload.code !== 200) {
    ...
    error.retryable = Number(payload.code) === 400 && /request failed|please retry|请求失败|重试/i.test(`${apiMessage} ${details}`);
    throw error;
  }
...
async function collectArticle({ url, apiKey, baseUrl, fetchImpl = fetch }) {
  ...
  const endpoints = [
    { url: `${base}/api/v1/wechat_mp/web/fetch_mp_article_detail_html?url=${encoded}`, mode: 'html', attempts: 4 },
  ];
